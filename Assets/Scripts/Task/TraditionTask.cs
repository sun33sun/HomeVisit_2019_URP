...
namespace HomeVisit.Task
{
	public class TraditionTask : SingletonMono<TraditionTask>
	{
		...
		private void Start()
		{
			EventCenter.GetInstance().AddEventListener<int>("访中过程考核结束", addScore =>
			{
				isExaming = false;
				totalScore += addScore;
			});
			EventCenter.GetInstance().AddEventListener<string>("<b>题目：</b>请输入学生的行为错在那些地方", newStr => { 
		...
		IEnumerator StartTask()
		{
			...
			yield return Interactive.Get<ObjColliderEvent>("老师坐下的位置").AreaHighlight();
			FemaleTeacher.Instance.canMove = false;
			yield return FemaleTeacher.Instance.SitDown(Interactive.Get("客厅坐位_老师").transform);
			...
			yield return RecordSpeech(new string[] { "校训", "学风", "作息", "学校特色", "校纪", "校规", "一日常规" });
			//聊班级
			mainPanel.NextTmp();
			...
			yield return RecordSpeech(new string[] { "学习环境", "家庭氛围", "合理分配时间", "和谐", "学习习惯", "生活习惯", "劳动能力" });
			yield return PlayAudio("母亲认可创造学习环境", 0, "嗯嗯，我们以后会为孩子创造一个安静的学习环境和学习氛围。");
			onVisitPanel.btnDialogue.gameObject.SetActive(false);
			//答疑惑
			mainPanel.NextTmp();
			yield return RecordSpeech(new string[] { "提供更多机会", "鼓励", "及时沟通", "相互理解" });
			//表达感谢
			mainPanel.NextStep();
			mainPanel.NextTmp();
		...
		IEnumerator LoadStudentInformationPaper()
		{
			...
			yield return WebKit.GetInstance().Read<List<SingleTitleData>>(Settings.PAPER + "Paper_OnVisit1.json", d =>{ datas = d; });
