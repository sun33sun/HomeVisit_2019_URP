...
namespace HomeVisit.Task
{
	public class SingleParentTask : SingletonMono<SingleParentTask>
	{
		...
		public IEnumerator StartTask()
		{
			...
			//展能力
			mainPanel.NextTmp();
			yield return StudentMather.Instance.Walk(Interactive.Get("学生蜷缩位置").transform);
			...
			onVisitPanel.btnDialogue.gameObject.SetActive(false);
			//聊班级
			buttonPanel.tmpTip.text = "请对班级概况，发展目标，任课老师进行简单的叙述，并说明彦威朋友不少却情绪低落。";
			yield return RecordSpeech(new string[] { "班级概况", "发展目标", "任课老师", "情绪低落", "朋友不少" });
			...
			yield return RecordSpeech(new string[] { "特别需求", "单亲", "发展目标" });
			...
			yield return FemaleTeacher.Instance.PlayAnim("StandUp");
			FemaleTeacher.Instance.canMove = true;
			yield return ObjHighlightClickCallBack(KeTingMen, true);
		}
