    ...
    public class Main : MonoBehaviour
    {
        IEnumerator Start()
        {
            ...
            yield return UIKit.PreLoadPanelAsync<MainPanel>(UILevel.Bg,
                prefabName: Settings.UI + Mainpanel_prefab.MAINPANEL);
            //获取信息
            yield return UIKit.PreLoadPanelAsync<GetInformationPanel>(
                prefabName: Settings.UI + Getinfornationpanel_prefab.GETINFORMATIONPANEL);
            //家访内容
            yield return UIKit.PreLoadPanelAsync<HomeVisitContentPanel>(
                prefabName: Settings.UI + Homevisitcontentpanel_prefab.HOMEVISITCONTENTPANEL);
            UIKit.HidePanel<HomeVisitContentPanel>();
            //家访形式
            yield return UIKit.PreLoadPanelAsync<HomeVisitFormPanel>(
                prefabName: Settings.UI + Homevisitformpanel_prefab.HOMEVISITFORMPANEL);
            //知识考核
            yield return UIKit.PreLoadPanelAsync<KnowledgeExamPanel>(
                prefabName: Settings.UI + Knowledgeexampanel_prefab.KNOWLEDGEEXAMPANEL);
            UIKit.HidePanel<KnowledgeExamPanel>();
            //实验报告
            TestReportPanelData reportPanelData = new TestReportPanelData();
            yield return WebKit.GetInstance().Read<List<ScoreReportData>>(
                Application.streamingAssetsPath + "/" + "DefaultReportData.json", d => { reportPanelData.datas = d; });
            yield return UIKit.PreLoadPanelAsync<TestReportPanel>(UILevel.Common,
                prefabName: Settings.UI + Testreportpanel_prefab.TESTREPORTPANEL);
            ...
            yield return UIKit.PreLoadPanelAsync<TestBriefPanel>(
                prefabName: Settings.UI + Testbriefpanel_prefab.TESTBRIEFPANEL);
            //最顶部页面
            yield return UIKit.PreLoadPanelAsync<TopPanel>(UILevel.PopUI,
                prefabName: Settings.UI + Toppanel_prefab.TOPPANEL);
        ...
        List<string> parentEducation = new List<string>()
            { "专科", "专科", "专科", "专科", "专科", "专科", "专科", "专科", "本科", "本科", "本科", "本科", "研究生", "研究生", "博士" };
        ...
        List<string> nations = new List<string>()
            { "汉", "满", "回", "蒙", "壮", "维吾尔", "汉", "汉", "汉", "汉", "汉", "汉", "汉", "汉", "汉" };
        ...
        string GenerateId(string birth)
        {
            ...
            if (int.Parse(births[2]) < 10)
                births[2] = "0" + births[2];
            string id = UnityEngine.Random.Range(100000, 999999) + births[0] + births[1] + births[2] +
        ...
        List<string> units = new List<string>()
            { "上海石化", "长江存储", "个体工商", "自由职业", "闵行一小", "宝山一中", "盐津铺子", "卫龙", "牧原食品", "", "金磨坊", "美的", "光明" };
